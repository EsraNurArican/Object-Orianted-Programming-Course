...
int main(int argc , char *argv[]){
	...
	if(argv[1]!=NULL){
	    ...
	    //N_Puzzle objeMain(m_boardsize); //Uses constructor with boardsize parameter.
	    								//Didn't use it because object is gone after the scope is finished

	    /*Initializing solvable board*/
	    objeMain.setSize(m_boardsize);
	...
	do{
	    ...
	    switch(action)
	    {
			...
			case 'S':
				...
				objeMain.shuffle(objeMain.getBoardObje().getColumn());
				objeMain.print();
				break;
			case 'Q':
				return -1;
